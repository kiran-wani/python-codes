...
# Now chisq and chisq/dof are needed.
ds=((y_new-y)/yerr)**2
chisq=sum(ds)
redchisq=chisq/(len(y)-2)
print(chisq,redchisq)
...
#=============================================
# This is the plot only!!!!

plt.style.use('classic')
fig,ax=plt.subplots()
ax.plot(x,y,'ro',label='data') 
errorbar(x,y,yerr=yerr,capsize=2,marker='o',color='g',ecolor='g',markerfacecolor='r',linestyle='None')
ax.plot(x,y_new,linestyle='--',label='Fitted line')
ax.minorticks_on()
ax.set_xlabel(r'Log temperature (T)',size="x-large")
ax.set_ylabel(r'Log Emissivity',size="x-large")
...
plt.savefig('jeewan.eps',format='eps',dpi=500)
plt.show()
